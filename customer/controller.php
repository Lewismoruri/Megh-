...
function doInsert(){
	...
	if(isset($_POST['submit'])){
			 ...
			 if(!isset($_POST['proid']) || (isset($_POST['proid']) && empty($_POST['proid']))){
			  echo "<script> alert('You are now successfully registered. Please wait for Account Approval Before Sign In.'); </script>";
						redirect(web_root."logout.php");
			 }else{
			 	$proid = $_GET['proid'];
			 	$id = mysqli_insert_id(); 
	...
	function doEdit(){
		if(isset($_POST['save'])){
			...
			$customer->update($_SESSION['CUSID']['CUSTOMERID']);
	...
	function doDelete(){
		...
		function processorder(){
			...
			$order->TRANSACTIONCODE	= $_POST['TRANSACTIONCODE']; 
	     	$order->create(); 
			...
			$summary->CUSTOMERID		= $_SESSION['CUSID']['CUSTOMERID'];
			$summary->ORDEREDNUM  	= $_POST['ORDEREDNUM'];  
			$summary->DELFEE  		= $_POST['PLACE']; 
			$summary->PAYMENTMETHOD	= $_POST['paymethod'];
			$summary->PROID		    = $_SESSION['gcCart'][$i]['productid']; 
			$summary->TRANSACTIONCODE	= $_POST['TRANSACTIONCODE'];
			$summary->LOCATIONDETAILS	= $_POST['LOCATIONDETAILS'];
			$summary->ORDEREDNUM		= $_POST['ORDEREDNUM']; 
			$summary->PAYMENT 		= $_POST['alltot'];
			$summary->ORDEREDSTATS 	= 'Waiting For Confirmation';
