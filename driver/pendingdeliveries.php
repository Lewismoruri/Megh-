...
<section class="content-header">
	<div class="content-header-left">
		<center><h4><font color="black">PENDING DELIVERIES</font></h4></center>
	</div>
</section>
<div class="container">
        ...
        <div class="box-body table-responsive table-bordered">
          <table id="example1" class="table table-bordered table-hover table-striped">
            ...
            <tbody>
            	<?php
            	$i=0;
            	$statement = $pdo->prepare("SELECT * FROM `tblsummary` where SHIPPING_STATUS='Goods on Transit'  AND DRIVER!='Not Assigned' and  SHIPPING_STATUS='Goods on Transit' AND DRIVER='".$_SESSION['EMAIL']."' ");
            	$statement->execute();
            	$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
            	foreach ($result as $row) {
            		$i++;
					...
					<tr class="<?php if($row['payment_status']=='Pending'){echo 'bg-r';}else{echo 'bg-g';} ?>">
                        ...
                        Delivery Status :<?php echo $row['ORDEREDREMARKS']; ?></b><hr>
                        <b> Shipping Status:<?php echo $row['SHIPPING_STATUS']; ?><br>
                        <?php
                                if($row['SHIPPING_STATUS']=='Goods on Transit'){
                                    ?>
                                    <a href="delivery-change-status.php?SUMMARYID=<?php echo $row['SUMMARYID']; ?>&task=Goods Delivered,&task2=Goods Delivered" class="btn btn-success btn-xs" style="width:100%;margin-bottom:4px;">Delivered</a>
                        ...
                        <?php
                           ...
                           foreach ($result1 as $row1) {
                                ...
                                echo '<br><b>Quantity:</b> '.$row1['ORDEREDQTY'].' Pcs<hr>';
                           }
                           ?>
                        </td>
