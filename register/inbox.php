  ...
  <div class="row">
    ...
    <button class="btn btn-sm btn-primary"><a href="sendmsg.php"><font color="white"><i class="fa fa-edit"></i>&nbsp;Compose New</font></a></button>
            ...
            <tbody>
            	<?php
            	$i=0;
            	$statement = $pdo->prepare("SELECT * FROM `tbmessages` where user_id='".$_SESSION['CUSID']['CUSTOMERID']."' or  recepient='".$_SESSION['CUSID']['CUSTOMERID']."' ");
            	$statement->execute();
            	$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
            	foreach ($result as $row) {
            		$i++;
					...
					<tr>
                        ...
                        <?php
                           $statement1 = $pdo->prepare("SELECT * FROM tbluseraccount WHERE USERID=?");
                           $statement1->execute(array($row['user_id']));
                           $result1 = $statement1->fetchAll(PDO::FETCH_ASSOC);
                           foreach ($result1 as $row1) {
                            
                            echo '<b>Sender :</b>'.$row1['U_ROLE'];
                           }
                           ?>
                          <?php
                           ...
                           $result1 = $statement1->fetchAll(PDO::FETCH_ASSOC);
                           foreach ($result1 as $row1) {
                                echo ' <b>Recepient :</b>'.$row1['U_ROLE'];
                           }
                           ?>
                              <?php
                                if($row['fname']==''){
