...
<section class="content-header">
	<div class="content-header-left">
		<center><h4>PENDING DRIVER ALLOCATIONS</h4></center>
	</div>
</section>
<div class="container">
        ...
        <div class="box-body table-responsive">
          <table id="example1" class="table table-bordered table-hover table-striped">
            ...
            <tbody>
				...
				WHERE   s.`ORDEREDNUM`=c.`ORDEREDNUM` and c.`PROID`=t.`PROID` and t.`CATEGID`=k.`CATEGID`  and ORDEREDSTATS='Confirmed' and SHIPPING_STATUS='Not Yet Shipped' AND DRIVER='Not Assigned' ORDER by SUMMARYID DESC");
            	$statement->execute();
            	$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
            	foreach ($result as $row) {
            		$i++;
					...
					<tr>
                       ...
                       <b >Price Per Quantity :</b> RF<?php echo $row['PROPRICE']; ?><br> <br>    
                       <font color="maroon"> <b >Total Price :RF </b> <?php echo $row['PROPRICE']*$row['ORDEREDQTY']; ?><br>    </font>
                        </td> 
                        <td>
                            <?php echo $row['DRIVER']; ?>                        
                        </td>
                        ...
                        <td>
                            <?php
                                if($row['SHIPPING_STATUS']=='Not Yet Shipped'){
                                    if($row['PAYMENT_STATUS']=='Approved'){
                                    if($row['DRIVER']=='Not Assigned'){
                                        if($row['ORDEREDSTATS']=='Confirmed'){
                                    ?>
                                    <a href="editform.php?SUMMARYID=<?php echo $row['SUMMARYID']; ?>" class="btn btn-success btn-xs" style="width:100%;margin-bottom:4px;">Allocate Driver</a>
