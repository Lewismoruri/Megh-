        ...
        <div class="box-body table-responsive">
          <table id="example1" class="table table-bordered table-hover table-striped">
            ...
            <tbody>
            	<?php
            	$i=0;
            	$statement = $pdo->prepare("SELECT * FROM tbl_bookings WHERE  photographer_status='On Process' and photographer='".$_SESSION['EMAIL']."'");
            	$statement->execute();
            	$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
            	foreach ($result as $row) {
            		$i++;
					...
					<tr class="<?php if($row['payment_status']=='Pending'){echo 'bg-r';}else{echo 'bg-g';} ?>">
                        ...
                        <td>
                        <?php
                            if($row['photographer_status']=='On Process') {
                                    ?>
                                    <a href="editform.php?id=<?php echo $row['id']; ?>&task=Completed" class="btn btn-success btn-md" >Service Delivered</a>
                                    <?php
                            }
                            ?>
                        </td>
