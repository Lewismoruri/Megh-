...
<?php                                      $sql = "SELECT *FROM requests where (status='Approved' or status='Goods On Transit') and inventoryStatus='Completed'  ";
$query = $dbh -> prepare($sql);
$query->execute();
...
<?php } else {?> <?php } ?>
                                            ...
                                            if($result->driver!=='')
 {?>
<a href="allocatedriver.php?id=<?php echo htmlentities($result->id);?>"  >  <button class="btn btn-primary btn-sm"> Dispatch</button>
<?php } else {?> <?php } ?>
