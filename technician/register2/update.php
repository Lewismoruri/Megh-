...
$statement = $pdo->prepare("UPDATE requestsmat SET status=? WHERE varietyName=?");
$statement->execute(array($_REQUEST['task'],$_REQUEST['varietyName']));

// Get the request details
$stat = $pdo->prepare("SELECT * FROM requestsmat WHERE varietyName=?");
...
if ($product) {
	...
	$insert->execute(array( $varietyName, $request['qty'], $request['supplier']));
