...
<section class="content">
  ...
  <div class="row">
    ...
    <button class="btn btn-sm btn-warning"><a href="sendmsg1.php"><font color="white"><i class="fa fa-edit"></i>&nbsp;Send To Staff</font></a></button>
</div><br>
			<thead>
			    <tr>
			        <th>#</th>
            ...
            <tbody>
            	<?php
            	$i=0;
            	$statement = $pdo->prepare("SELECT * FROM `tbmessages` where user_id='".$_SESSION['EMAIL']."' or  recepient='".$_SESSION['EMAIL']."' ");
            	$statement->execute();
            	$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
            	foreach ($result as $row) {
            		$i++;
