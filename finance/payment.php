...
<section class="content-header">
	<div class="content-header-left">
		<center><h4><font color="black">PENDING ORDER PAYMENT</font></h4></center>
	</div>
</section>
<div class="container">
        ...
        <div class="box-body table-responsive table-bordered">
          <table id="example1" class="table table-bordered table-hover table-striped">
            ...
            <tbody>
            	<?php
            	$i=0;
            	$statement = $pdo->prepare("SELECT * FROM `tblsummary` where PAYMENT_STATUS='Paid Waiting For Finance Approval' ");
            	$statement->execute();
            	$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
            	foreach ($result as $row) {
            		$i++;
					...
					<tr class="<?php if($row['payment_status']=='Pending'){echo 'bg-r';}else{echo 'bg-g';} ?>">
                        ...
                        <td>
                        <b><font color="red">Order Num :<?php echo $row['ORDEREDNUM']; ?></font><br>
                        ...
                        <?php
                           ...
                           foreach ($result1 as $row1) {
                                ...
                                echo '<br><b>Price Per Qty:</b> Ksh '.$row1['PROPRICE'];
                                echo '<br><b>Quantity:</b> '.$row1['ORDEREDQTY'].'<hr>';
                           }
                           ?>
                        </td>
                        <td>                   
                       <b> Total Price :Ksh </b><?php echo $row['PAYMENT']-$row['DELFEE']; ?><br>
                        <b>Delivery Fee :Ksh </b><?php echo $row['DELFEE']; ?></b><br>
                        <b>Payment Method :</b><?php echo $row['PAYMENTMETHOD']; ?></b><br><br>
                        <b>Date Paid :</b><?php echo $row['ORDEREDDATE']; ?></b><br>
                        <b>Transaction Id :</b><?php echo $row['TRANSACTIONCODE']; ?></b><br>
                        <b><font color="red">GRAND TOTAL:Ksh </b><?php echo $row['PAYMENT']; ?></font><br><br>
                        <?php echo $row['PAYMENT_STATUS']; ?>
                            <br>
                            <?php
                                if($row['PAYMENT_STATUS']=='Paid Waiting For Finance Approval'){
                                    ?>
                                    <a href="payment-change-status.php?SUMMARYID=<?php echo $row['SUMMARYID']; ?>&task=Approved" class="btn btn-success btn-xs" style="width:100%;margin-bottom:4px;">Approve</a>
                                   <!-- <a href="editform.php?SUMMARYID=<?php echo $row['SUMMARYID']; ?>" class="btn btn-danger btn-xs" style="width:100%;margin-bottom:4px;">Reject</a>-->
