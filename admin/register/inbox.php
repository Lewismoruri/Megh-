...
<section class="content">
  ...
  <div class="row">
    ...
    <button class="btn btn-sm btn-warning"><a href="sendmsg1.php"><font color="white"><i class="fa fa-edit"></i>&nbsp;Send To Supplier</font></a></button>
</div><br>
			<thead>
			    <tr>
			        <th>#</th>
            ...
            <tbody>
            	<?php
            	$i=0;
            	$statement = $pdo->prepare("SELECT * FROM `tbmessages` where user_id='".$_SESSION['USERID']."' or  recepient='".$_SESSION['USERID']."' ");
            	$statement->execute();
            	$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
            	foreach ($result as $row) {
            		$i++;
					...
					<tr>
                        ...
                        <?php
                           ...
                           foreach ($result1 as $row1) {
                            ...
                            echo '<b>Sender :</b>'.$row1['FNAME'] .$row1['LNAME'];
                           }
                           ?>
                          <?php
                           $statement1 = $pdo->prepare("SELECT * FROM tblcustomer WHERE CUSTOMERID=?");
                           ...
                           $result1 = $statement1->fetchAll(PDO::FETCH_ASSOC);
                           foreach ($result1 as $row1) {
                                echo ' <b>Recepient :</b>'.$row1['FNAME'] .$row1['LNAME'];
                           }
                           ?><br>
                           <?php
                          ...
                          $result1 = $statement1->fetchAll(PDO::FETCH_ASSOC);
                          foreach ($result1 as $row1) {
                           echo '<b>Sender :</b>'.$row1['U_NAME'].'&nbsp;-' .$row1['U_ROLE'];
                          }
                          ?><br>
                             <?php
                          $statement1 = $pdo->prepare("SELECT * FROM tbluseraccount WHERE EMAIL=?");
                          ...
                          $result1 = $statement1->fetchAll(PDO::FETCH_ASSOC);
                          foreach ($result1 as $row1) {
                               echo '<b>Recepient :</b>'.$row1['U_NAME'] .'&nbsp;-' .$row1['U_ROLE'];
                          }
                          ?><br>
                              <?php
                                if($row['user']==''){
