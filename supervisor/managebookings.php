...
<section class="content">
  ...
  <div class="row">
    <div class="col-md-12">
      ...
      <a class="btn btn-info" href="index.php?dashboard">Back</a>
        <div class="box-body table-responsive">
          <table id="example1" class="table table-bordered table-hover table-striped">
            ...
            <tbody>
            	<?php
            	$i=0;
            	$statement = $pdo->prepare("SELECT * FROM tbl_bookings where status='Approved' and photographer_status='Completed' and sup_remark='' ORDER by id DESC");
            	$statement->execute();
            	$result = $statement->fetchAll(PDO::FETCH_ASSOC);							
            	foreach ($result as $row) {
            		$i++;
...
<b>Total Charges:</b> Ksh <?php echo $row['charges']*$row['qty']+($row['fee']); ?><br>
                    ...
                    <td>
                        ...
                        <td>
                        <?php
                                    if($row['sup_confirm']=='Not Yet Confirmed' && $row['photographer_status']=='Completed' ){
                                    ?>
